...
with st.sidebar.expander("💰 Income & Savings"):
    ...
    savings_goal = st.number_input("Savings Goal (₹)", min_value=savings, step=10000)
...
# Predict Future Net Worth using Compound Interest
def predict_net_worth(years=5, rate=0.08):
    future_net_worth = net_worth_now * ((1 + rate) ** years) + sum([net_savings * ((1 + rate) ** (years - i)) for i in range(years)])
...
st.subheader("📈 Net Worth Growth Over 5 Years")
years = np.arange(1, 6)
net_worth_projection = [predict_net_worth(y) for y in years]
st.line_chart(pd.DataFrame({"Year": years, "Net Worth": net_worth_projection}).set_index("Year"))
